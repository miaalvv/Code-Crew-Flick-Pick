...
export async function POST(req: Request) {
  ...
  // 4) Round IS complete so compute global matches
  ...
  // if session is NOT finished, move party into RESULTS phase
  // and stamp results_started_at exactly once (idempotent-ish).
  if (!isSessionFinished) {
    const { data: partyRow, error: partyFetchErr } = await supabase
      .from("parties")
      .select("session_state, results_started_at")
      .eq("id", party_id)
      .maybeSingle();

    if (partyFetchErr) {
      return NextResponse.json(
        { ok: false, error: partyFetchErr.message },
        { status: 400 }
      );
    }

    // Only stamp if not already in results (prevents re-stamping if multiple clients call completeRound)
    if (partyRow?.session_state !== "results") {
      ...
      if (partyUpdateErr) {
        return NextResponse.json(
          { ok: false, error: partyUpdateErr.message },
          { status: 400 }
        );
      }
    }
