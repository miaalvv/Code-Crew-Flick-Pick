...
export default function WatchPreferencesPage() {
  ...
  const loadTiles = async () => {
      ...
      const fetchNames = async (table: string, column: string, limit = 3) => {
        ...
        return (data as Record<string, string>[]).map((r) => r[column]).filter(Boolean) as string[];
      };
      ...
      const fmt = (items: string[]) =>
        items.length === 0 ? 'Not set' : items.slice(0, 3).join(', ') + (items.length > 3 ? ` +${items.length - 3}` : '');
      ...
      const tilesData: TileSummary[] = [
        ...
        },
        {
          ...
          summary:
            dur?.min_duration != null && dur?.max_duration != null ? `${dur.min_duration}–${dur.max_duration} min` : 'Not set',
          items:
    ...
    <div className="mt-1.5 space-y-6">
      <div className="flex flex-wrap items-center gap-2 mb-2">
        <button
          ...
          className="inline-flex items-center rounded-full border border-slate-600 bg-slate-800 px-4 py-2 text-[11px] font-semibold text-slate-100 shadow-sm hover:bg-slate-700 transition"
        >
          ← Back
        </button>
        <button
          ...
          disabled={resetting}
          className="inline-flex items-center rounded-full border border-rose-500/60 bg-rose-500/10 px-4 py-2 text-[11px] font-semibold text-rose-100 shadow-sm shadow-rose-500/20 hover:bg-rose-500/20 hover:border-rose-400 transition disabled:opacity-60"
        >
      ...
      <section className="rounded-3xl border border-slate-700/70 bg-slate-900/80 p-6 shadow-xl shadow-black/40 space-y-3">
        ...
        <h1 className="text-2xl font-semibold text-slate-50">Tune your watch preferences</h1>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {(loading ? Array.from({ length: 6 }) : tiles).map((tile, idx) => (
          <a
            key={tile?.href || idx}
            href={tile?.href || '#'}
          ...
          >
            <div className="flex items-center justify-between">
              <div className="text-base font-semibold text-slate-50">{tile?.label || 'Loading…'}</div>
              <span className="text-[11px] text-pink-200">{tile ? 'Edit →' : '...'}</span>
            </div>
            <p className="mt-2 text-xs text-slate-300">{tile?.desc || 'Loading preferences…'}</p>
            <p className="mt-3 text-[11px] text-slate-200">
              {tile?.summary || (loading ? 'Fetching saved choices…' : 'Not set')}
            </p>
      ...
      {confirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border border-slate-700 bg-slate-900/90 p-6 shadow-2xl shadow-black/50">
            <h3 className="text-lg font-semibold text-slate-50">Reset preferences?</h3>
            <p className="mt-2 text-sm text-slate-300">
              This will clear your streaming services, genres, actors, directors, keywords,
              durations, studios, and decades. You can re-select them anytime.
            </p>
            <div className="mt-4 flex items-center gap-3">
              <button
                onClick={resetAll}
                disabled={resetting}
                className="inline-flex items-center rounded-full bg-rose-500 px-4 py-2 text-sm font-semibold text-white shadow-md shadow-rose-500/30 hover:bg-rose-400 transition disabled:opacity-60"
              >
              ...
              <button
                ...
                disabled={resetting}
                className="inline-flex items-center rounded-full border border-slate-600 bg-slate-800 px-4 py-2 text-sm font-semibold text-slate-100 hover:bg-slate-700 transition"
              >
                Cancel
              </button>
            </div>
